...
read_enumeration_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (die->child != NULL && !die_is_declaration (die, cu))
	   ...
	   child_die = sibling_die (child_die))
	{
	  if (child_die->tag == DW_TAG_enumerator)
	      ...
	      if (name)
		{
		  ...
		  value = dwarf2_get_attr_constant_value (attr, 0);
		  ...
		  if ((num_fields % DW_FIELD_ALLOC_CHUNK) == 0)
		    {
		      int new_size = num_fields + DW_FIELD_ALLOC_CHUNK;

		      fields = (struct field *)
			xrealloc (fields, new_size * sizeof (struct field));
		      /* Interning examines all the bits, so ensure
			 holes are set to a known value.  */
		      memset (&fields[num_fields], 0,
			      DW_FIELD_ALLOC_CHUNK * sizeof (struct field));
		    }

		  FIELD_NAME (fields[num_fields]) = name;
		  FIELD_TYPE (fields[num_fields]) = NULL;
		  SET_FIELD_BITPOS (fields[num_fields], value);
      ...
      if (num_fields)
	{
	  ...
	  if (flag_enum)
	    type_flags |= TYPE_FLAG_FLAG_ENUM;

	  type = intern_enum_type (length, type_flags, (char *) name,
				   objfile, num_fields, fields);
...
process_enumeration_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (die->child != NULL && !TYPE_NFIELDS (this_type))
    {
      struct die_info *child_die;
      struct symbol *sym;
      struct field *fields = NULL;
...
read_unspecified_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  type = intern_type (TYPE_CODE_VOID, 1, 0, dwarf2_name (die, cu),
		      cu->objfile, NULL);
