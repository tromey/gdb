...
/* Trait type for the varobj hash table.  */
struct varobj_traits
{
  using value_type = varobj *;

  static bool equals (const varobj *lhs, const varobj *rhs)
  { return lhs == rhs; }

  static bool equals (const varobj *lhs, const char *name)
  { return lhs->obj_name == name; }

  static bool is_empty (const varobj *v)
  { return v == nullptr; }

  static size_t hash (const varobj *v)
  { return htab_hash_string (v->obj_name.c_str ()); }
...
static gdb::traited_hash_table<varobj_traits> varobj_table;
...
{
  auto insert_pair = varobj_table.insert (var);
