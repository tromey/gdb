...
tui_source_window::do_scroll_vertical (int num_to_scroll)
{
  if (!m_content.empty ())
	...
	line_no = 1;
      ...
      cursal.line = line_no;
      find_line_pc ({ cursal.symtab, cursal.objfile }, cursal.line, &cursal.pc);
				       ...
				       CORE_ADDR *addr_p)
{
  ...
  *gdbarch_p = m_gdbarch;
  find_line_pc ({ cursal.symtab, cursal.objfile },
		m_start_line_or_addr.u.line_no, addr_p);
}
