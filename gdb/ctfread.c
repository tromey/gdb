...
using tid_type_map = gdb::unordered_map<ctf_id_t, struct type *>;

static const registry<objfile>::key<tid_type_map> ctf_tid_key;
...
{
  tid_type_map *htab = ctf_tid_key.get (of);
  if (htab == nullptr)
    htab = ctf_tid_key.emplace (of);

  htab->emplace (tid, typ);
  return typ;
}
