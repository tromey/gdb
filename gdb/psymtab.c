...
psymtab_to_symtab (struct objfile *objfile, struct partial_symtab *pst)
{
  ...
  struct compunit_symtab *pcs = pst->get_compunit_symtab (objfile);
  if (pcs != nullptr)
    return pcs;
  ...
  if (psymtab->readin_p (objfile))
    {
      ...
      gdb_print_host_address (psymtab->get_compunit_symtab (objfile),
			      outfile);
      fprintf_filtered (outfile, ")\n");
    }
...
partial_symtab::read_dependencies (struct objfile *objfile)
{
  for (int i = 0; i < number_of_dependencies; i++)
    if (!dependencies[i]->readin_p (objfile) && dependencies[i]->user == NULL)
      {
