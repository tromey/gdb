...
maintenance_check_psymtabs (const char *ignore, int from_tty)
{
  ...
  for (objfile *objfile : current_program_space->objfiles ())
	...
	if (ps->raw_text_high () != 0
	    && (ps->text_low (objfile) < XBLOCK_START (objfile, b)
		|| ps->text_high (objfile) > XBLOCK_END (objfile, b)))
	  {
	    printf_filtered ("Psymtab ");
	    puts_filtered (ps->filename);
	    ...
	    fputs_filtered (paddress (gdbarch, XBLOCK_START (objfile, b)),
			    gdb_stdout);
	    printf_filtered (" - ");
	    fputs_filtered (paddress (gdbarch, XBLOCK_END (objfile, b)),
			    gdb_stdout);
	    printf_filtered ("\n");
	  }
      }
