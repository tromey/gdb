  ...
  if (sym != NULL)
    {
      ...
      objfile = symbol_objfile (sym);
      pc = XBLOCK_ENTRY_PC (objfile, SYMBOL_BLOCK_VALUE (sym));
  ...
  do
	...
	pc = gdbarch_skip_prologue_noexcept (gdbarch, pc);
	 ...
	 line is still part of the same function.  */
	  ...
	  && (sym ? (XBLOCK_ENTRY_PC (objfile,
				      SYMBOL_BLOCK_VALUE (sym)) <= start_sal.end
		     && start_sal.end < XBLOCK_END (objfile,
...
find_function_alias_target (bound_minimal_symbol msymbol)
{
  ...
  block_symbol sym = find_pc_function (func_addr);
  if (sym.symbol != NULL
      && SYMBOL_CLASS (sym.symbol) == LOC_BLOCK
      && BLOCK_ENTRY_PC (SYMBOL_BLOCK_VALUE (sym.symbol)) == func_addr)
    return sym.symbol;
...
find_gnu_ifunc (const symbol *sym)
{
  ...
  CORE_ADDR address = XBLOCK_ENTRY_PC (objfile, SYMBOL_BLOCK_VALUE (sym));
