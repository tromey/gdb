...
public:
  ...
  cooked_gdb_index (dwarf2_per_objfile *per_objfile,
		    std::unique_ptr<cooked_index_worker> worker,
		    int version)
    : cooked_index (per_objfile, std::move (worker)),
      version (version)
  { }
			       ...
			       mapped_gdb_index *index)
{
  ...
  index->shard->install_addrmap (&mutable_map);
}
   ...
   get_gdb_index_contents_dwz_ftype get_gdb_index_contents_dwz)
{
  ...
  auto worker = std::make_unique<gdb_index_worker> (per_objfile,
						    std::move (map));
  auto idx = std::make_unique<cooked_gdb_index> (per_objfile,
						 std::move (worker),
						 version);
