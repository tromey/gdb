...
{
  auto iter = m_type_map.find (type);
...
compile_instance::insert_type (struct type *type, gcc_type gcc_type)
{
  /* The type might have already been inserted in order to handle
     recursive types.  */
  auto iter = m_type_map.find (type);
  if (iter != m_type_map.end () && (*iter).second != gcc_type)
    error (_("Unexpected type id from GCC, check you use recent enough GCC."));

  m_type_map.insert (type, gcc_type);
}
...
compile_instance::insert_symbol_error (const struct symbol *sym,
				       const char *text)
{
  m_symbol_err_map.insert (sym, text);
}
...
{
  auto iter = m_symbol_err_map.find (sym);
  if (iter == m_symbol_err_map.end () || (*iter).second.empty ())
    return;
