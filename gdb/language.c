      ...
      if (strcmp (lang->la_name, language) == 0)
	{
	  ...
	  if (lang->la_language == language_auto)
	  ...
	  else
	    {
	      ...
	      set_range_case ();
	      expected_language = real_current_language;
	      return;
	    }
	}
    }
...
{
  if (range_mode == range_mode_auto)
    range_check = real_current_language->la_range_check;

  if (case_mode == case_mode_auto)
    case_sensitivity = real_current_language->la_case_sensitivity;
}
...
set_language (enum language lang)
{
  ...
  prev_language = real_current_language->la_language;
  real_current_language = languages[lang];
  set_range_case ();
  return prev_language;
}

/* True if the initial language has been set, and false if it still
   must be set.  */
