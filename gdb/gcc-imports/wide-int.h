...
#undef UNARY_FUNCTION
  ...
  template <typename T>
  UHOST_WIDE_INT extract_uhwi (const T &, unsigned int, unsigned int);
...
public:
  ...
  UHOST_WIDE_INT to_uhwi (unsigned int) const;
  UHOST_WIDE_INT to_uhwi () const;
  ...
  UHOST_WIDE_INT ulow () const;
  UHOST_WIDE_INT uhigh () const;
...
/* Return THIS as an UHOST_WIDE_INT, zero-extending from
   PRECISION.  If THIS does not fit in PRECISION, the information
   is lost.  */
template <typename storage>
inline UHOST_WIDE_INT
...
generic_wide_int <storage>::sign_mask () const
{
  ...
  UHOST_WIDE_INT high = this->get_val ()[len - 1];
  if (!is_sign_extended)
    {
...
namespace wi
{
  ...
  /* template <> */
  /* struct int_traits <LONGEST> */
  /*   : public primitive_int_traits <LONGEST, true> {}; */

  /* template <> */
  /* struct int_traits <ULONGEST> */
  /*   : public primitive_int_traits <ULONGEST, false> {}; */
...
namespace wi
{
  ...
  hwi_with_prec uhwi (UHOST_WIDE_INT, unsigned int);
...
inline wi::hwi_with_prec
wi::uhwi (UHOST_WIDE_INT val, unsigned int precision)
{
...
/* Return true if X fits in an UHOST_WIDE_INT with no loss of
   precision.  */
template <typename T>
inline bool
...
wi::eq_p (const T1 &x, const T2 &y)
{
  ...
  if (LIKELY (yi.len == 1))
	...
	return xi.val[0] == 0;
      ...
      UHOST_WIDE_INT diff = xi.val[0] ^ yi.val[0];
...
wi::ltu_p (const T1 &x, const T2 &y)
{
  ...
  if (STATIC_CONSTANT_P (yi.len == 1 && yi.val[0] >= 0))
    return xi.len == 1 && xi.to_uhwi () < (UHOST_WIDE_INT) yi.val[0];
  if (STATIC_CONSTANT_P (xi.len == 1 && xi.val[0] >= 0))
    return yi.len != 1 || yi.to_uhwi () > (UHOST_WIDE_INT) xi.val[0];
  ...
  if (LIKELY (xi.len + yi.len == 2))
    {
      UHOST_WIDE_INT xl = xi.to_uhwi ();
      UHOST_WIDE_INT yl = yi.to_uhwi ();
      return xl < yl;
    }
...
wi::add (const T1 &x, const T2 &y)
{
  ...
  /* If the precision is known at compile time to be greater than
    ...
    {
      UHOST_WIDE_INT xl = xi.ulow ();
      UHOST_WIDE_INT yl = yi.ulow ();
      UHOST_WIDE_INT resultl = xl + yl;
      val[0] = resultl;
...
wi::extract_uhwi (const T &x, unsigned int bitpos, unsigned int width)
{
  ...
  UHOST_WIDE_INT res = xi.elt (start);
  res >>= shift;
  if (shift + width > HOST_BITS_PER_WIDE_INT)
    {
      UHOST_WIDE_INT upper = xi.elt (start + 1);
