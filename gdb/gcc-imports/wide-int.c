...
#define HOST_HALF_WIDE_INT int32_t
#define UHOST_HALF_WIDE_INT uint32_t
...
#define BYTES_BIG_ENDIAN 0
	...
	offset = BYTES_BIG_ENDIAN ? (buffer_len - 1) - byte : byte;
      ...
      value = (UHOST_WIDE_INT) buffer[offset];
...
wi::to_mpz (const wide_int_ref &x, mpz_t result, signop sgn)
{
  ...
  if (wi::neg_p (x, sgn))
	...
	t[i] = ~v[i];
      if (excess > 0)
	t[len - 1] = (UHOST_WIDE_INT) t[len - 1] << excess >> excess;
	...
	t[i] = v[i];
      t[len - 1] = (UHOST_WIDE_INT) v[len - 1] << excess >> excess;
      mpz_import (result, len, -1, sizeof (HOST_WIDE_INT), 0, 0, t);
    }
...
wi::from_mpz (const_tree type, mpz_t x, bool wrap)
{
  ...
  /* Determine the number of UHOST_WIDE_INTs that are required
	       ...
	       signop sgn, wi::overflow_type *overflow)
{
  ...
  UHOST_WIDE_INT o1 = 0;
  UHOST_WIDE_INT x = 0;
  UHOST_WIDE_INT carry = 0;
  UHOST_WIDE_INT old_carry = 0;
  UHOST_WIDE_INT mask0, mask1;
  unsigned int i;
  ...
  for (i = 0; i < len; i++)
    {
      o0 = i < op0len ? (UHOST_WIDE_INT) op0[i] : mask0;
      o1 = i < op1len ? (UHOST_WIDE_INT) op1[i] : mask1;
      x = o0 + o1 + carry;
      val[i] = x;
      old_carry = carry;
  ...
  if (len * HOST_BITS_PER_WIDE_INT < prec)
	...
	*overflow
  ...
  else if (overflow)
    {
      ...
      if (sgn == SIGNED)
	{
	  UHOST_WIDE_INT x2 = (val[len - 1] ^ o0) & (val[len - 1] ^ o1);
	  if ((HOST_WIDE_INT) (x2 << shift) < 0)
	    {
	      if (o0 > (UHOST_WIDE_INT) val[len - 1])
		*overflow = wi::OVF_UNDERFLOW;
	      else if (o0 < (UHOST_WIDE_INT) val[len - 1])
...
static void
wi_unpack (UHOST_HALF_WIDE_INT *result, const HOST_WIDE_INT *input,
...
wi_pack (HOST_WIDE_INT *result,
	 const UHOST_HALF_WIDE_INT *input,
	 unsigned int in_len, unsigned int precision)
{
  ...
  while (i + 1 < in_len)
    {
      result[j++] = ((UHOST_WIDE_INT) input[i]
		     | ((UHOST_WIDE_INT) input[i + 1]
		  ...
		  wi::overflow_type *overflow, bool high)
{
  ...
  if ((high || needs_overflow)
      ...
      if (needs_overflow)
	{
	  if (sgn == SIGNED)
	    {
	      if ((HOST_WIDE_INT) r2 != sext_hwi (r2, prec))
  ...
  for (j = 0; j < half_blocks_needed; j++)
	...
	{
	  t = ((UHOST_WIDE_INT)u[i] * (UHOST_WIDE_INT)v[j]
	       + r[i + j] + k);
  ...
  if (sgn == SIGNED && (high || needs_overflow))
      ...
      if (wi::neg_p (op1))
	{
	  ...
	  for (i = 0; i < half_blocks_needed; i++)
	    {
	      t = (UHOST_WIDE_INT)r[i + half_blocks_needed]
		- (UHOST_WIDE_INT)v[i] - b;
	       ...
	       signop sgn, wi::overflow_type *overflow)
{
  ...
  UHOST_WIDE_INT borrow = 0;
  UHOST_WIDE_INT old_borrow = 0;
  ...
  if (len * HOST_BITS_PER_WIDE_INT < prec)
	...
	*overflow = (sgn == UNSIGNED && borrow) ? OVF_UNDERFLOW : OVF_NONE;
  ...
  else if (overflow)
    {
      ...
      if (sgn == SIGNED)
	{
	  UHOST_WIDE_INT x2 = (o0 ^ o1) & (val[len - 1] ^ o0);
	  if ((HOST_WIDE_INT) (x2 << shift) < 0)
	    {
	      if (o0 > o1)
...
static void
divmod_internal_2 (UHOST_HALF_WIDE_INT *b_quotient,
		   UHOST_HALF_WIDE_INT *b_remainder,
		   UHOST_HALF_WIDE_INT *b_dividend,
		   UHOST_HALF_WIDE_INT *b_divisor,
		   int m, int n)
{
  /* The "digits" are a HOST_HALF_WIDE_INT which the size of half of a
     ...
     machines.  */
  UHOST_WIDE_INT b
    = (UHOST_WIDE_INT)1 << HOST_BITS_PER_HALF_WIDE_INT;
  UHOST_WIDE_INT qhat;   /* Estimate of quotient digit.  */
  UHOST_WIDE_INT rhat;   /* A remainder.  */
  UHOST_WIDE_INT p;      /* Product of two digits.  */
  HOST_WIDE_INT t, k;
  int i, j, s;
  ...
  if (n == 1)
      ...
      for (j = m - 1; j >= 0; j--)
	{
	  ...
	  k = ((k * b + b_dividend[j])
	       - ((UHOST_WIDE_INT)b_quotient[j]
		  * (UHOST_WIDE_INT)b_divisor[0]));
	}
      b_remainder[0] = k;
      return;
    }
		     ...
		     wi::overflow_type *oflow)
{
  ...
  if (sgn == UNSIGNED
    ...
    {
      UHOST_WIDE_INT o0 = dividend.to_uhwi ();
      UHOST_WIDE_INT o1 = divisor.to_uhwi ();
  ...
  if (small_shift == 0)
  ...
  else
    {
	 ...
	 consecutive input blocks.  */
      ...
      for (unsigned int i = skip; i < len; ++i)
	{
	  UHOST_WIDE_INT x = safe_uhwi (xval, xlen, i - skip);
  ...
  if (small_shift == 0)
  ...
  else
    {
	 ...
	 The last block is a right shift of the last block in XVAL.  */
      UHOST_WIDE_INT curr = safe_uhwi (xval, xlen, skip);
      for (unsigned int i = 0; i < len; ++i)
	{
