...
inferior_call_waitpid (ptid_t pptid, int pid)
{
  ...
  /* Get the waitpid_fn.  */
  if (lookup_bound_minimal_symbol ("waitpid").minsym != NULL)
    waitpid_fn = find_function_in_inferior ("waitpid", &waitpid_objf);
  if (!waitpid_fn
      && lookup_bound_minimal_symbol ("_waitpid").minsym != NULL)
...
checkpoint_command (const char *args, int from_tty)
{
  ...
  if (lookup_bound_minimal_symbol ("fork").minsym != NULL)
    fork_fn = find_function_in_inferior ("fork", &fork_objf);
  if (!fork_fn)
    if (lookup_bound_minimal_symbol ("_fork").minsym != NULL)
      fork_fn = find_function_in_inferior ("fork", &fork_objf);
  if (!fork_fn)
