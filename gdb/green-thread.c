...
/* All the providers that have been registered so far.  */
static gdb::unordered_set<green_provider *> providers;
...
/* A target layer that enables user-space (aka "green") threads.  */
...
struct green_thread_target final : public target_ops
{
  ...
  {
    if (!ptid.lwp_p () && ptid.tid_p ())
      {
	/* We assume that the concrete implementation of the green
	   threads will track this and mark the thread as exited.  */
	return true;
      }
...
private:
  ...
  /* A helper function that finds the green thread object
     corresponding to PTID.  If PTID does not name a green thread,
     nullptr is returned.  */
			   ...
			   target_wait_flags options)
{
  ...
  ptid_t event_ptid = beneath ()->wait (ptid, status, options);
  if (event_ptid == minus_one_ptid || event_ptid == null_ptid
  ...
  for (green_provider *provider : providers)
    {
      ptid_t result = provider->current_green_thread ();
      if (result != null_ptid)
	return result;
    }
...
namespace {  /* For now, don't clash with ravenscar-thread.c.  */
...
green_thread_target::enable_btrace (thread_info *tp,
				    const struct btrace_config *conf)
{
  /* If TP is a green thread, try to apply the request to the
     underlying thread.  */
  process_stratum_target *proc_target = current_inferior ()->process_target ();
  green_thread *gthr = dynamic_cast<green_thread *> (tp->priv.get ());
  if (gthr != nullptr)
    {
      ptid_t underlying = gthr->underlying_thread ();
      if (underlying == null_ptid)
	error (_("Can't enable btrace for the current thread"));

      tp = proc_target->find_thread (underlying);
    }

  return proc_target->enable_btrace (tp, conf);
}
...
/* Implement the core_of_thread target_ops method.  */
...
struct thread_info *
add_green_thread (green_thread_up gth, green_provider *provider)
{
