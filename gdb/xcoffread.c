	...
	if (symbol->n_zeroes)
	  {
		...
		p = obstack_alloc (SYMBOL_OBSTACK (objfile), E_SYMNMLEN + 1);
...
#define	SYMBOL_DUP(SYMBOL1, SYMBOL2)	\
  (SYMBOL2) = (struct symbol *)		\
  	obstack_alloc (SYMBOL_OBSTACK (objfile), sizeof (struct symbol)); \
  *(SYMBOL2) = *(SYMBOL1);


#define	SYMNAME_ALLOC(NAME, ALLOCED)	\
  ((ALLOCED) ? (NAME) : obstack_copy0 (SYMBOL_OBSTACK (objfile), \
				       (NAME), strlen (NAME)))
...
init_stringtab (bfd *abfd, file_ptr offset, struct objfile *objfile)
{
  ...
  strtbl = (char *) obstack_alloc (SYMBOL_OBSTACK (objfile), length);
  xcoff->strtbl = strtbl;
		   ...
		   int number_dependencies, int textlow_not_set)
{
  ...
  for (i = 0; i < num_includes; i++)
	...
	allocate_psymtab (include_list[i], objfile);
      ...
      subpst->read_symtab_private = obstack_alloc (SYMBOL_OBSTACK (objfile),
						   sizeof (struct symloc));
...
xcoff_initial_scan (struct objfile *objfile, int symfile_flags)
{
	...
	if (secp)
	  {
	    ...
	    if (length)
	      {
		debugsec = obstack_alloc (SYMBOL_OBSTACK (objfile), length);
  ...
  info->symtbl = obstack_alloc (SYMBOL_OBSTACK (objfile), size);
