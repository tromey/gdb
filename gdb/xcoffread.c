...
static void read_xcoff_symtab (struct objfile *, struct legacy_partial_symtab *);
...
static struct legacy_partial_symtab *this_symtab_psymtab;
...
static void
read_xcoff_symtab (struct objfile *objfile, struct legacy_partial_symtab *pst)
{
...

static void
xcoff_psymtab_to_symtab_1 (struct objfile *objfile,
...
static void
xcoff_read_symtab (struct legacy_partial_symtab *self, struct objfile *objfile)
{
  if (self->readin)
    {
      fprintf_unfiltered
		     ...
		     const char *filename, int first_symnum)
{
  struct legacy_partial_symtab *result =
    new legacy_partial_symtab (objfile,
			       filename,
  ...
  result->do_read_symtab = xcoff_read_symtab;
...
static struct legacy_partial_symtab *
xcoff_end_psymtab (struct objfile *objfile, struct legacy_partial_symtab *pst,
		   const char **include_list, int num_includes,
		   int capping_symbol_number,
		   struct legacy_partial_symtab **dependency_list,
  ...
  if (number_dependencies)
	...
	= objfile->partial_symtabs->allocate_dependencies (number_dependencies);
      memcpy (pst->dependencies, dependency_list,
	      number_dependencies * sizeof (struct legacy_partial_symtab *));
    }
  else
    pst->dependencies = 0;
		   ...
		   struct objfile *objfile)
{
  ...
  pst = (struct legacy_partial_symtab *) 0;
  ...
  dependencies_used = 0;
  dependency_list =
    (struct legacy_partial_symtab **) alloca (dependencies_allocated *
				       sizeof (struct legacy_partial_symtab *));
